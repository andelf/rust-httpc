    ...
    fn fill_buf(&mut self) {
        if self.read_pos > 0 {
            unsafe {
                ...
                //self.buf.copy_memory(self.buf.slice(self.read_pos, self.write_pos));
            }
...
impl<R:Reader> Reader for IconvReader<R> {
    fn read(&mut self, buf: &mut [u8]) -> IoResult<uint> {
        ...
        match err {
            EILSEQ => {
                println!("An invalid multibyte sequence has been encountered in the input.");
                return Err(io::standard_error(io::InvalidInput));
            }
            EINVAL => {
                println!("An incomplete multibyte sequence has been encountered in the input.");
                return Ok(nwrite);
            }
            E2BIG => {
                println!("There is not sufficient room at *outbuf.");
                return Ok(nwrite);
            }
            0 => {
                return Ok(nwrite);
...
//                     })
//             });
//         IconvReader { inner: r, h: handle , eof: false, buf: ~[],
//                       inbut:  ~[],
//                       output: ~[]

//                       //               unsafe { buf.set_len(65536) };
//                       //               buf },
//                       // output: { let ~mut buf = ~vec::with_capacity(65536);
//                       //               unsafe { buf.set_len(65536) };
//                       //               buf },
//                       // input_pos: 0, output_pos: 0
//         }
//     }
// }
...
//         println!("out buf len = {:?}", out_bytes_left);
//         let mut tbuf = vec::with_capacity::<u8>(out_size);

//         if self.buf.len() != 0 {
//             let moved = tbuf.copy_from(self.buf);
//             if moved < self.buf.len() {
//                 let remains = self.buf.slice_from(moved).into_owned();
//                 self.buf = remains;
//             } else {
//                 self.buf = ~[];
//             }
//             unsafe { tbuf.set_len(moved) }
//         }
//         let tbuf_remains = in_size - tbuf.len();

//         match self.inner.push_bytes(&mut tbuf, tbuf_remains) {
//             Ok(()) => {
//                 let in_size = tbuf.len();
...
//                             return Err(io::standard_error(io::InvalidInput));
//                         }
//                         E2BIG => println!("There is not sufficient room at *outbuf."),
...
//                     }
//                     if in_bytes_left > 0 {
//                         let writen = out_size - out_bytes_left as uint;
//                         let remains = tbuf.slice(in_size - in_bytes_left as uint, in_size);
//                         println!("remains => {:?}", remains);
//                         self.buf = remains.into_owned();
